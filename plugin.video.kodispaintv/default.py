...
addon = xbmcaddon.Addon('plugin.video.kodispaintv')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.kodispaintv-%s]: %s" %(addon_version, string), xbmc.LOGNOTICE)
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        if isinstance(data,BeautifulSOAP):
            ...
            elif data.find('menus_info'):
                media_info = data.menus_info
        if media_info:
            source_media = {}
        ...
        xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,New source added.,5000,"+icon+")")
        if not url is None:
...
def getData(url,fanart, data=None):
    ...
    #print 'xxxxxxxxxxsoup',soup
        if len(soup('channels')) > 0 and addon.getSetting('donotshowbychannels') == 'false':
            ...
            # ** get page epg and store in local file ** #
            tepg = None
            media_info = None
            if soup.find('channels_info'):
                media_info = soup('channels_info')
            elif soup.find('menus_info'):
                media_info = soup('menus_info')

            if media_info:            
                try:
                    if media_info[0].epg:
                        epg = media_info[0].epg.string
                        reg_item = media_info[0]('epg_regex')
                        regexs = parse_regex(reg_item)
                        if '$doregex' in epg and not getRegexParsed==None:
                            tepg, setres=getRegexParsed(regexs, epg)
                            #print 'xxxxxxxxxxxxxxpageEPG',  tepg, setres
                    ...
                    #traceback.print_exc()
                    print 'error getting EPG page data: %s'%e.message
...
#                print channel
            ...
            addon_log('No Channels: getMenus')
            getMenus(soup('menu'),fanart)
    else:
        parse_m3u(soup)

# borrow from https://github.com/enen92/P2P-Streams-XBMC/blob/master/plugin.video.p2p-streams/resources/core/livestreams.py
# This will not go through the getMenus functions ( means you must have ready to play url, no regex)
def parse_m3u(data):
    content = data.rstrip()
...
def getChannelMenus(name,url,fanart):
        soup = getSoup(url)
        channel_list = soup.find('channel', attrs={'name' : name.decode('utf-8')})
        menus = channel_list('menu')
        try:
        ...
        getMenus(menus,fanArt)

def getSubChannelMenus(name,url,fanart):
        soup = getSoup(url)
        channel_list = soup.find('subchannel', attrs={'name' : name.decode('utf-8')})
        menus = channel_list('submenu')
        getMenus(menus,fanart)

def getMenus(menus,fanart,dontLink=False):
        total = len(menus)
        addon_log('Total Menus: %s' %total)
        ...
        for menu in menus:
            ...
            applyblock='false'
            try:
                applyblock = menu('parentalblock')[0].string
            except:
            ...
            try:
                if menu('epg'):
                    addon_log('xxxxxxxxxxxxxxmenuEPG')
                    # ** basic regex on epg_url tag for epg added to menu name ** #                    
                    if menu.epg_url and menu.epg_url.string:
                        try:
                            epg_url = menu.epg_url.string
                            epg_regex = menu.epg_regex.string
                    ...
                    # ** py function block regex to generate epg for menu plot ** #                    
                    elif menu('epg')[0].string > 1:
                        epg = menu.epg.string                        

                        if '$doregex' in epg:
                            reg_item = menu('epg_regex')

                            #if page tag is not provided use epg generated in channel infp or menus info
                            if not reg_item[0].page.string or reg_item[0].page.text == '':
                                filename = 'LSProPageEPG.txt'
                                filenamewithpath = os.path.join(functions_dir,filename)
                                reg_item[0].page.string = filenamewithpath

                            regexs = parse_regex(reg_item)                   
                            iepg, setres=getRegexParsed(regexs, epg)

                        # ** or add static epg to menu name ** #
                        else: name += getepg(menu('epg')[0].string)                    
                ...
                #traceback.print_exc()
                print 'Error getting menu EPG: %s'%e.message


            try:
                url = []
                if len(menu('link')) >0:
                    #print 'menu link', menu('link')

                    for i in menu('link'):
                        if not i.string == None:
                            url.append(i.string)

                elif len(menu('sportsdevil')) >0:
                    for i in menu('sportsdevil'):
                        if not i.string == None:
                            sportsdevil = 'plugin://plugin.video.SportsDevil/?mode=1&amp;menu=catcher%3dstreams%26url=' +i.string +'%26videoTitle=' +name
                            referer = menu('referer')[0].string
                            if referer:
                                #print 'referer found'
                    ...
                    for i in menu('ftv'):
                        ...
                        url.append(ftv)
                elif len(menu('urlsolve')) >0:                    
                    for i in menu('urlsolve'):
                        if not i.string == None:
                            resolver = i.string +'&mode=19'
                            url.append(resolver)
                elif len(menu('inputstream')) >0:                    
                    for i in menu('inputstream'):
                        if not i.string == None:
                            istream = i.string + '&mode=20'
                            url.append(istream)
                elif len(menu('slproxy')) >0:
                    for i in menu('slproxy'):
                        if not i.string == None:
                ...
                continue
            try:
                isXMLSource = menu('externallink')[0].string
            except: pass
            ...
            if isXMLSource:
                ...
                isXMLSource=False
            try:
                isJsonrpc = menu('jsonrpc')[0].string
            except: pass
            if isJsonrpc:
                ...
                isJsonrpc=False
            try:
                thumbnail = menu('thumbnail')[0].string
                if thumbnail == None:
                    raise
                thumbnail=processPyFunction(thumbnail)
            except:
            ...
            try:
                desc = menu('info')[0].string
                if desc == None:
                    raise
            except:
                # ** use menu epg in plot if present ** #
                if iepg:
                    desc = iepg
                else: desc = ''

            try:
                genre = menu('genre')[0].string
                if genre == None:
                    raise
            except:
                genre = ''
            ...
            try:
                ...
                addon_log('There was a problem adding menu - '+name.encode('utf-8', 'ignore'))

def parse_regex(reg_menu):
                try:
                    regexs = {}
                    for i in reg_menu:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def get_saw_rtmp(page_value, referer=None):
    ...
    str_pattern=r"(eval\(function\(p,a,c,k,e,(?:r|d).*)"
...
#        print 'final value is ',r
    ...
    rtmp=re_me(page_value,r'streamer\'.*?\'(.*?)\'\)')
    playpath=re_me(page_value,r'file\',\s\'(.*?)\'')
...
def get_leton_rtmp(page_value, referer=None):
    ...
    if page_value.startswith("http"):
        page_value= getUrl(page_value,headers=referer)
    str_pattern=r"var a = (.*?);\s*var b = (.*?);\s*var c = (.*?);\s*var d = (.*?);\s*var f = (.*?);\s*var v_part = '(.*?)';"
...
def findAndReplaceWord(source_str, word_to_find,replace_with):
    ...
    if len(splits)>1:
        ...
        for current_split in splits:
            ...
            #if its first one and menu is blank then check next menu is valid or not
...
def getFavorites():
        menus = json.loads(open(favorites).read())
        total = len(menus)
        for i in menus:
            name = i[0]
            url = i[1]
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,resolveurl donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
#                    print sepate
            ...
            for i in mu_playlist:
                menu += 1
                info = xbmcgui.ListItem('%s) %s' %(str(menu),name))
...
def download_file(name, url):
        ...
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('KodiSpainTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('KodiSpainTV', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from KodiSpainTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to KodiSpainTV Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#               print 'setting as folder in link'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false" and '$$LSPlayOnlyOne$$' not in playlist[0] :
                ...
                name = name + '[COLOR magenta] (' + str(len(playlist)) + ' menus )[/COLOR]'
                play_list = True
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from KodiSpainTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to KodiSpainTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
if mode==None:
    ...
    if not url.startswith("plugin://plugin") or not any(x in url for x in g_ignoreSetResolved):#not url.startswith("plugin://plugin.video.f4mTester") :
        ...
        menu = xbmcgui.ListItem(path=url)        
        if not setres:
            xbmc.Player().play(url)
        else: 
            xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, menu)
    else:
...
#                print 'newcopy',newcopy
                ...
                #ln+='\n<menu>%s\n%s</menu>'%(listrepeatT.encode("utf-8"),regex_xml)   
                try:
...
#            ln+='</menu>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    addon_log("setResolvedUrl")
    menu = xbmcgui.ListItem(name)
    if '$$lic' in url:
        url,lic = url.split('$$lic=')
        lic = urllib.unquote_plus(lic)
        if '{SSM}' not in lic:
            lic += '||R{SSM}|'
        menu.setProperty('inputstream.adaptive.license_type', 'com.widevine.alpha')
        menu.setProperty('inputstream.adaptive.license_key', lic)
    if '|' in url:
        url,strhdr = url.split('|')
        menu.setProperty('inputstream.adaptive.stream_headers', strhdr)
        menu.setPath(url)
    if '.m3u8' in url:
        menu.setProperty('inputstreamaddon', 'inputstream.adaptive')
        menu.setProperty('inputstream.adaptive.manifest_type', 'hls')
        menu.setMimeType('application/vnd.apple.mpegstream_url')
        menu.setContentLookup(False)
   
    elif '.mpd' in url or 'format=mpd' in url:
        menu.setProperty('inputstreamaddon', 'inputstream.adaptive')
        menu.setProperty('inputstream.adaptive.manifest_type', 'mpd')
        menu.setMimeType('application/dash+xml')
        menu.setContentLookup(False)
    
    elif '.ism' in url:
        menu.setProperty('inputstreamaddon', 'inputstream.adaptive')
        menu.setProperty('inputstream.adaptive.manifest_type', 'ism')
        menu.setMimeType('application/vnd.ms-sstr+xml')
        menu.setContentLookup(False)
    menu.setPath(url)
    xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, menu)
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
