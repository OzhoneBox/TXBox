...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title = 'Por interprete, dirección', action = 'list_labels', url = host, group = 'Etiquetas' ))
...
def list_labels(item): 
    ...
    bloque = scrapertools.find_single_match(data, ">" + str(item.group) + "<(.*?)<div class='clear'></div>")
    matches = scrapertools.find_multiple_matches(bloque, "<span class='label-size label-size-(.*?)'>.*?href='(.*?)'.*?>(.*?)</a>")

    i = 0

    for tip, url, title in matches:
        if not item.grupo:
        ...
        elif title == 'FILMOGRAFÍAS': continue
        elif title == 'COLECCIÓN': continue
        ...
        if item.group == 'Etiquetas':
            if tip == '1' or tip == '4': continue
